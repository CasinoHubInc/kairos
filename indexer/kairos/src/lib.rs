...
use pb::starknet::v1::*;
use crate::abi::kairos_contract::Event as StakcastEvent;
...
fn map_kairos_events(transactions: Transactions) -> Result<Events, substreams::errors::Error> {
    ...
    for transaction in transactions.transactions_with_receipt {
        ...
        for event in data_events {
            ...
            if let kairos_event = StakcastEvent::try_from(emitted_event).unwrap() {
                let event_json = serde_json::to_string(&kairos_event).unwrap();
                let event = Event {
